...
## useful functionality of some tidyverse functions
...
#### `mutate()`
To create a new column quickly
...
``` r
# Createa new column which combines Crop and Treatment 
...
### the pipe `%>%`
We can also combine multiple functions together. This is an important concept and is very powerful.It helps to visualize what you are dping to your data at each step.  Basically the pipe allows you to input the previous data from the right side of the pipe into the function on the left (or next line).

The data from previous step is transferred to the next step.
...
To find things like means and standard deviations/errors
...
groups of functions and packages also all work together
...
![](Data_wrangling_notes_files/figure-html/unnamed-chunk-10-1.png)<!-- -->


#### Joining
To match up data by rows based on some criteria
They include 
 left_join() - keep all rows of X and add matching rows from Y.Any rows in Y that dont match X are excluded
...
### Pivoting
Useful for converting from wide to long format and back again.We can do this using `pivot_longer()` and `pivot_wider()`
...
We can now easily calculate the difference between fungicide and control and plot it
...
![](Data_wrangling_notes_files/figure-html/unnamed-chunk-14-1.png)<!-- -->
